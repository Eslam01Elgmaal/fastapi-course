...
async def render_todo_page(request: Request):
    try:
        ...
        return templates.TemplateResponse(request, "add_todo.html", {"user": user})
    except:
        return redirect_to_login()
...
async def render_edit_todo_page(request:Request, todo_id: int, db:db_dependency):
    try:
        ...
        return templates.TemplateResponse(request, "edit_todo.html", {"todo":todo, "user": user})
...
async def render_delete_todo_page(request: Request, todo_id: int, db: db_dependency):
    try:
        user = await get_current_user(request.cookies.get("access_token"))
        if user is None:
            return redirect_to_login()

        todo = db.query(Todos).filter(Todos.id == todo_id).delete()

        return templates.TemplateResponse(request, "delete_todo.html", {"todo":todo, "user": user})
...
async def read_by_id(user: user_dependency,db: db_dependency, todo_id: int = Path(gt=0)):
    ...
    if todo_model is not None:
        return todo_model
    raise HTTPException(status_code= 404, detail= 'Todos not found yet')
